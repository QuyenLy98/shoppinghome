...
export default function PlaceOrderScreen() {
  ...
  const round2 = (num) => Math.round(num * 100 + Number.EPSILON) / 100; //123.23 => 123.23
  ...
  const placeOrderHandler = async () => {
    try {
      ...
      const { data } = await axios.post(
        'https://server-app-led2.onrender.com/api/orders',
        {
  ...
  return (
    <div>
      ...
      <h1 className="my-3">Xem trước đơn hàng</h1>
      <Row>
        <Col md={8}>
          <Card className="mb-3">
            <Card.Body>
              <Card.Title>Thông tin người mua hàng</Card.Title>
              <Card.Text>
                <strong>Tên: </strong>
                  ...
                  <ListGroup.Item key={item._id}>
                    <Row className="align-items-center">
                      <Col md={6}>
                        ...
                        ></img>{' '}
                        <Link
                          to={`https://server-app-led2.onrender.com/product/${item.slug}`}
                        >
                          {item.name}
                        </Link>
                      </Col>
